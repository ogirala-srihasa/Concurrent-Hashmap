(** QCheck-STM State Machine Test for Split-Ordered Hash Map
...
let run cmd sut =
  ...
  | Insert (k, v) ->
    ...
    Res (unit, ())
  | Find k ->
    Res (option int, SO.find sut k)
  | Remove k ->
    Res (bool, SO.remove sut k)
  | Size ->
    Res (int, SO.size sut)
...
module Spec = struct
  ...
  let precond = precond
  let run = run
  let init_sut () = SO.create ~capacity:8 ()
  let cleanup _ = ()
...
let run_sequential_test () =
  ...
  let seq_test = Seq.agree_test ~count:1000 ~name:"Split-ordered hashmap sequential" in
...
let run_concurrent_test () =
  ...
  in
  let conc_test =
    QCheck.Test.make ~retries:10 ~count:200 ~name:"Split-ordered hashmap concurrent"
      arb_cmds_par
