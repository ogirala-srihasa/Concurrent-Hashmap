...
module (defined in \texttt{hashmap\_intf.ml}), which must supply a type \texttt{t},
...
% -----------------------------------------------------------------------------
\subsubsection{Striped locking hash map (\texttt{striped\_hashmap.ml})}
% -----------------------------------------------------------------------------
...
% -----------------------------------------------------------------------------
\subsubsection{Split-ordered list hash map (\texttt{split\_ordered\_hashmap.ml})}
% -----------------------------------------------------------------------------
...
Both \texttt{test\_striped.ml} and \texttt{test\_split\_ordered.ml} have implementation of the following tests: sequential operations, error handling,
...
\texttt{qcheck\_stm\_striped.ml} and \texttt{qcheck\_stm\_split\_ordered.ml}
...
\texttt{qcheck\_lin\_striped.ml} and \texttt{qcheck\_lin\_split\_ordered.ml} each test generates 1\,000 random
...
detection. The manual concurrent tests (\texttt{test\_striped.ml},
\texttt{test\_split\_ordered.ml}) were used as the TSAN workloads since they
